# 🚀 Быстрый старт

## 1. Тест сервера вручную

```bash
# Активация виртуального окружения
.\venv\Scripts\Activate.ps1  # Windows
source venv/bin/activate      # Linux/Mac

# Запуск сервера
python server.py
```

**Ожидаемый вывод:**

```text
INFO - Запуск Gemini Media Analyzer MCP сервера...
```

Нажмите `Ctrl+C` для остановки.

## 2. Подключение к MCP клиенту

### Claude Desktop

1. Отредактируйте файл конфигурации (см. [Конфигурация](configuration.md))
2. Перезапустите Claude Desktop
3. Проверьте доступность сервера в Claude

### Cursor

1. Откройте Command Palette (`Ctrl+Shift+P`)
2. Введите "MCP Settings"
3. Добавьте конфигурацию сервера
4. Перезапустите Cursor

## 3. Первый анализ

### Анализ изображения
В вашем MCP клиенте спросите:

```text
Проанализируй это изображение: C:\Users\ВашеИмя\Pictures\photo.jpg
```

### Анализ аудио
В вашем MCP клиенте спросите:

```text
Проанализируй это аудио: C:\Users\ВашеИмя\Music\audio.mp3
```

## 4. Пользовательские запросы

### Анализ изображения
```text
Проанализируй C:\Photos\sunset.jpg и опиши цвета и настроение
```

### Анализ аудио
```text
Суммаризируй это аудио: C:\Users\ВашеИмя\Recordings\meeting.mp3
```

## 5. Первая генерация

### Генерация изображения
В вашем MCP клиенте спросите:

```text
Сгенерируй: "кот в шляпе"
```

## Частые команды

| Задача | Команда |
|--------|---------|
| Базовый анализ изображения | `Проанализируй изображение: /путь/к/изображению.jpg` |
| Базовый анализ аудио | `Проанализируй аудио: /путь/к/аудио.mp3` |
| Генерация изображения | `Сгенерируй: "кот в шляпе"` |
| Кастомный запрос изображения | `Опиши эмоции на /путь/к/фото.jpg` |
| Транскрипция аудио | `Транскрибируй это аудио: /путь/к/аудио.wav` |
| Технический анализ | Используйте `system_instruction_name: "technical"` |

## Следующие шаги

- [💡 Руководство по использованию](usage.md) - Детальные инструкции и примеры

---

**Проблемы?** Проверьте [Решение проблем](troubleshooting.md)
